...
import { ChartBarIcon, ChatBubbleOvalLeftEllipsisIcon, EllipsisHorizontalCircleIcon, HeartIcon, ShareIcon, TrashIcon } from "@heroicons/react/24/outline";
...
export default function Comment({ comment, commentId, originalPostId }) {
  ...
  return (
    <div className="flex p-3 cursor-pointer border-b border-gray-200 pl-20">
      {/* user image */}
      <img className="h-11 w-11 rounded-full mr-4" src={comment?.userImg} alt="profile" />
      {/* right side */}
      <div className="flex-1">
        ...
        <div className="flex justify-between text-gray-500 p-2">
          ...
          <div className="flex items-center">
            {hasLiked ? (
              <HeartIconFilled onClick={likeComment} className="h-9 w-9 hoverEffect p-2 hover:text-red-600 hover:bg-red-100" />
            ) : (
